...
namespace JBooth.MegaSplat
{
   ...
   public class MegaSplatUtilities 
   {
      ...
      static void RenderMeshPreview (Material mat) 
      {
         ...
         sPru.camera.transform.position = new Vector3(0, 0, -4);
         sPru.camera.transform.LookAt(Vector3.zero);

         sPru.lights[0].transform.rotation = Quaternion.Euler(30,30,30f);
         sPru.lights[0].intensity = 1.0f;
         sPru.lights[0].color = Color.white;
         sPru.lights[0].type = LightType.Directional;
         sPru.lights[0].enabled = true;
